...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName = prompt('Введите название остановки');
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новое название остановки', oldTaskName);
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Остановка '${taskName}' будет удалена. Продолжить?`)) return;
  ...
  render() {
    ...
    header.innerHTML = 'Номер маршрута:<br>' + this.tlName;
    tasklist.appendChild(header);

    const time = document.createElement('header');
    time.classList.add('tm-tasklist-header');
    time.innerHTML = 'Тип маршрута:<br>' + this.tlTime;
